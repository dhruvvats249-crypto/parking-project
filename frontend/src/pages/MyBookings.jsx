          ...
          <div className="booking-row">
            <div>
              ...
              <div className="meta" style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
                <code style={{ fontSize: "0.75rem", color: "var(--ink-muted)", background: "var(--bg)", padding: "2px 6px", borderRadius: "4px" }}>{b.id}</code>
                <button
                  className="btn btn-ghost"
                  style={{ padding: "4px 8px", fontSize: "0.75rem" }}
                  onClick={() => {
                    navigator.clipboard.writeText(b.id);
                    setError("Booking ID copied!");
      ...
      {subscriptions.map((s) => (
        <div className="booking-row" key={s.id}>
          <div>
            ...
            <div className="meta" style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
              <code style={{ fontSize: "0.75rem", color: "var(--ink-muted)", background: "var(--bg)", padding: "2px 6px", borderRadius: "4px" }}>{s.id}</code>
              <button
                className="btn btn-ghost"
                style={{ padding: "4px 8px", fontSize: "0.75rem" }}
                onClick={() => {
                  navigator.clipboard.writeText(s.id);
                  setError("Subscription ID copied!");
                  setTimeout(() => setError(null), 2000);
                }}
